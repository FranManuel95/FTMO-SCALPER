...
class SignalEngine:
    ...
    def analyze(self, df_5m: pd.DataFrame) -> TradeSetup:
        ...
        # Si no hay crossover, intentar PULLBACK
    ...
    def _check_pullback(self, df: pd.DataFrame) -> Signal:
        """
        Precio toca la EMA21 en tendencia establecida (EMA9 > EMA21 desde ≥3 velas).
        Genera entradas más frecuentes sin esperar cruces nuevos.
        """
        if len(df) < 5:
            return Signal.NONE
        curr  = df.iloc[-1]
