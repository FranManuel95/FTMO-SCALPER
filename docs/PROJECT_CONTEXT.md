...
## Portfolio live activo (11 estrategias)
...
Proyección conservadora (descuento correlación 35%): ~$9,500/6m sobre $10k (~1.6%/mes). Phase 1 FTMO en ~3-4 semanas. Frecuencia: ~90-105 trades/mes en total.
...
| Pullback | EURGBP 1H | 4.40 | BoE/ECB convergencia anula edge periódicamente |
| Pullback | USDCHF 1H | 2.67 | SNB intervenciones gap 300+ pips (trail no protege) |
| FVG ICT | XAUUSD 1H | 1.57 | 4ª estrategia en XAUUSD = correlación en laterales |
| Pullback LONG-only | AUDUSD 1H | 5.76 | Solo 7–8 trades/6m, estadísticamente inútil |
...
- **Asian Session ORB en JPY cruces es la estrategia estrella.** USDJPY PF 3.22, EURJPY PF 9.36. JPY establece rango asiático, EUR/USD rompe en apertura London. WFE > 1 en ambas.
- **BoJ gap risk es inmanejable con trailing stop 1H.** GBPJPY descartado por esto — puede caer 700 pips en horas. GBPJPY London ORB 15M sí funciona porque las posiciones cierran en ~30 min (intraday, no overnight).
...
**Próximas fronteras potenciales:**
- Añadir EURGBP pullback al live en periodo de divergencia BoE/ECB (5/6, PF 4.40)
- Añadir USDCHF pullback si se acepta el riesgo SNB con monitoreo manual (5/6, PF 2.67)
- Regenerar validation JSON de USDJPY Asian ORB a trail=0.5 (JSON en disco es del sweep trail=0.2)
- Investigar EURUSD Asian ORB 1H (no testado aún)
- Investigar estrategias en 4H para pares ya validados en 1H
