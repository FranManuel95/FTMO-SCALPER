...
def main():
    rr_ratio_values = [1.25, 1.40, 1.50]
    breakeven_trigger_r_values = [0.8, 1.0, 1.2]
    max_hold_m5_bars_values = [18, 24]
    breakout_buffer_atr_values = [0.02, 0.03, 0.05]
    body_pct_min_values = [0.20, 0.25, 0.30]
    regime_adx_min_values = [20, 24, 26]
    min_h1_ema_spread_atr_values = [0.10, 0.15, 0.20]
    ...
    print(f"\nLanzando grid global M5 con {len(configs)} combinaciones...\n")
    ...
    out_csv = "backtest/results/xau_m5_global_grid_results.csv"
    pd.DataFrame(results).to_csv(out_csv, index=False)

    print("\n" + "=" * 220)
    print("TOP 25 RESULTADOS - GRID GLOBAL XAUUSD M5 SELL_ONLY")
    print("=" * 220)
    print(
        f"{'RANK':<6}"
        f"{'RR':>6}"
