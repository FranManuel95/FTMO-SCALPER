# backtest/backtester_xau_m5.py
# XAUUSD ORB v1 - M5 Native
# - Datos base: M5
# - Construye H1 desde M5
# - ORB sobre M5
# - Filtro H1 causal (sin lookahead)
# - Ejecución y gestión sobre M5
# - Selector de dirección: BOTH / BUY_ONLY / SELL_ONLY
# - Filtro opcional por fechas
# - Diagnóstico interno
#
# Diseñado para histórico largo cuando no dispones de M1 completo.
...
class XAUM5Backtester:
    DATA_PATH_M5 = "backtest/data/XAUUSD_5M.csv"
    ...
    # Salida por estancamiento, opcional
    ...
    def _read_ohlc_csv(self, path: str) -> pd.DataFrame:
        if not os.path.exists(path):
            raise FileNotFoundError(
                ...
                "Necesitas backtest/data/XAUUSD_5M.csv"
            )
    ...
    def load_data(self):
        df_m5 = self._read_ohlc_csv(self.DATA_PATH_M5)
        df_h1 = self._resample_ohlc(df_m5, "1h")

        print(
            f"XAUUSD M5: {len(df_m5)} | H1: {len(df_h1)} | "
            f"Precio: {df_m5['close'].iloc[-1]:.2f} | Mode: {self.trade_mode} | "
            f"Rango: {df_m5.index[0]} -> {df_m5.index[-1]}"
        )
    ...
    # =========================================================
    # Simulación intrabar M5
    # =========================================================
    def _simulate_trade_m5(
        self,
        df_m5: pd.DataFrame,
        ...
        if future.empty:
            return signal_time, entry, "timeout", False, 0.0, 0.0, 0.0, 0
        ...
        for ts, bar in future.iterrows():
            ...
            if signal_side == "BUY":
                ...
                current_worst_r = (entry - low) / risk_distance
            else:
                current_best_r = (entry - low) / risk_distance
                current_worst_r = (high - entry) / risk_distance

            best_r = max(best_r, current_best_r)
            worst_r = max(worst_r, current_worst_r)
            ...
            if signal_side == "BUY":
                if low <= stop:
                    return (
                        ts,
                        stop,
                        "breakeven" if stop >= entry else "sl",
                        be_armed,
                        best_r,
                        worst_r,
            ...
            if (
                ...
                if signal_side == "BUY":
                    realized_r = (close - entry) / risk_distance
                else:
                    realized_r = (entry - close) / risk_distance
        ...
        if signal_side == "BUY":
            realized_r = (last_close - entry) / risk_distance
        else:
            realized_r = (entry - last_close) / risk_distance
    ...
    def run(self) -> BacktestResult:
        ...
        for _, day_df in grouped:
            ...
            for i in range(1, len(session) - 1):
                ...
                if self.USE_REGIME_FILTER:
                    ...
                    long_context = bull_regime and adx_h1 >= self.REGIME_ADX_MIN
                    short_context = bear_regime and adx_h1 >= self.REGIME_ADX_MIN
                ...
                trades.append(
                    {
                        ...
                        "mfe_r": round(float(best_r), 4),
                        "mae_r": round(float(worst_r), 4),
                        "best_r": round(float(best_r), 4),
                        "worst_r": round(float(worst_r), 4),
                        "orb_high": orb_high,
                        "orb_low": orb_low,
                        "atr_at_entry": atr,
                        "adx_h1_at_entry": adx_h1,
        ...
        start_label = pd.Timestamp(df_m5.index[0]).strftime("%Y%m%d_%H%M") if len(df_m5) else "na"
        end_label = pd.Timestamp(df_m5.index[-1]).strftime("%Y%m%d_%H%M") if len(df_m5) else "na"

        label_suffix = f"_{self.run_label}" if self.run_label else ""
        trades_csv_path = (
            f"{results_dir}/xau_m5_trades_{self.trade_mode}_{start_label}_{end_label}{label_suffix}.csv"
        )
